...
class PromptInjectionFilter:
    ...
    def __init__(self):
        self.dangerous_patterns = [
            ...
            # r'(what|give|reveal|pass|send|return|dump|print)\s*(\S+\s*)*(the|your)?\s*(\S+\s*)*(flag?)'  # these words are short, its hard to scramble them
        ]
    ...
    def normalize_keywords(self, text: str) -> str:
        ...
        for token in tokens:
            # If token has numbers/symbols inside, skip it (optional)
            if re.search(r'[a-zA-Z]+[0-9!@#$%^&*()_+\-=\[\]{};\'":\\|,.<>\/?]+[a-zA-Z]+', token):
                continue
            # Correct typos or leet variants
            match = get_close_matches(token.lower(), self.keywords, n=1, cutoff=0.8)
            if match:
